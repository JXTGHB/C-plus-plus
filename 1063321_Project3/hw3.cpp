...
void tomasulo(string inst[][20],int address);
...
int main()
{
	...
	tomasulo(inst,address);
	system("pause");
}
void tomasulo(string inst[][20],int address)
{   //此版本capture&dispatch可在同一cycle做
	/*add,sub 2cycle,mul 10cycle,div 40cycle*/
	int rf[6]; 
	for (int i = 1, j = 0; i < 6; i++) {  //給予rf初始值
		rf[i] = j;
		j += 2;          
	}
	int cycle = 1;
	int current = 0; //存現在讀到哪個inst
	bool end = false;  //判斷程式是否結束
	bool issue = false;  //有issue的話current才需要++
	string add_rs[4][6] = {}; //第五項存放入的cycle   //第三項存最後的值要存回哪一個register
	string mul_rs[10][6] = {}; //第五項存放入的cycle
	string rat[6] = {};
	string add_bf[10];  //儲存add buffer值
	string mul_bf[10];	//儲存mul buffer值
	
	while (end != true)
	{
//-----把值寫入RS
		issue = false;  //如果add有讀到，就要做初始化
		if (inst[current][1] == "addi" || inst[current][1] == "add" || inst[current][1] == "sub") {
			for (int i = 1; i <= 3; i++) {  //確認每一個rs是不是空的，是空的就把值放進去，放完break
				if (add_rs[i][0] == "") {
					if (inst[current][1] == "add") {
						add_rs[i][0] = '+';  
						if (rat[(int)(inst[current][3][1] - '0')] == "") {
							add_rs[i][1] = to_string(rf[(int)(inst[current][3][1] - '0')]);
						}
						else {
							add_rs[i][1] = rat[(int)(inst[current][3][1] - '0')];
						}
						if (rat[(int)(inst[current][4][1] - '0')] == "") {
							add_rs[i][2] = to_string(rf[(int)(inst[current][4][1] - '0')]);
						}
						else {
							add_rs[i][2] = rat[(int)(inst[current][4][1] - '0')];
						}
						add_rs[i][3] = inst[current][2][1];  //第三項存最後的值要存回哪一個register
					}
					if (inst[current][1] == "addi") {
						add_rs[i][0] = '+';
						if (rat[(int)(inst[current][3][1] - '0')] == "") {
							add_rs[i][1] = to_string(rf[(int)(inst[current][3][1] - '0')]);
						}
						...
						add_rs[i][2] = inst[current][4];
						add_rs[i][3] = inst[current][2][1];
					}
					if (inst[current][1] == "sub") {
						add_rs[i][0] = '-';
						if (rat[(int)(inst[current][3][1] - '0')] == "") {
							add_rs[i][1] = to_string(rf[(int)(inst[current][3][1] - '0')]);
						}
					...
					add_rs[i][5] = to_string(cycle);  //紀錄是在哪個cycle issue進來的
					rat[(int)(inst[current][2][1] - '0')] = "RS" + to_string(i); //紀錄register現在裡面放RS幾
					current++; //有讀入inst才要做++
					issue = true;  //為了預防current改變後，又去讀下一組inst，所以把issue設成true
					break;
				}
			}
		}
		if ((inst[current][1] == "mul" || inst[current][1] == "div")&&issue==false) {
			for (int i = 4; i < 6; i++) {
				if (mul_rs[i][0] == "") {
 					if (inst[current][1] == "mul") {
						mul_rs[i][0] = '*';
						if (rat[(int)(inst[current][3][1] - '0')] == "") {
							mul_rs[i][1] = to_string(rf[(int)(inst[current][3][1] - '0')]);
						}
						else {
							mul_rs[i][1] = rat[(int)(inst[current][3][1] - '0')];
						}
						if (rat[(int)(inst[current][4][1] - '0')] == "") {
							mul_rs[i][2] = to_string(rf[(int)(inst[current][4][1] - '0')]);
						}
						else {
							mul_rs[i][2] = rat[(int)(inst[current][4][1] - '0')];
						}
						mul_rs[i][3] = inst[current][2][1];
					}
					if (inst[current][1] == "div") {
						mul_rs[i][0] = '/';
						if (rat[(int)(inst[current][3][1] - '0')] == "") {
							mul_rs[i][1] = to_string(rf[(int)(inst[current][3][1] - '0')]);
						}
					...
					mul_rs[i][5] = to_string(cycle);
					rat[(int)(inst[current][2][1] - '0')] = "RS" + to_string(i);
					current++;
					break;
				}
			}
...
//----dispatch&放入buffer 
		for (int i = 1; i <= 3; i++) {
			if (add_rs[i][1] == "")  //rs裡面如果根本沒值，就直接跳下一個
				continue;
			if (add_rs[i][1][0] != 'R'&&add_rs[i][2][0] != 'R'&&stoi(add_rs[i][5]) < cycle&&add_bf[0] == "") {
			//rs裡面兩項都已經是數字，且不是issue的那個cycle，且buffer是空的，就可以做dispatch
				add_bf[0] = "RS" + to_string(i);   //第0項存我是在rs幾做計算的
				add_bf[1] = add_rs[i][1];
				add_bf[2] = add_rs[i][0];  //存運算符號
				add_bf[3] = add_rs[i][2];
				add_bf[4] = add_rs[i][3];  //存要放回RF哪一項  如果是存回R1就存1
				add_bf[5] = to_string(cycle + 2);  //第五項存第幾個cycle應該要離開buffer放回rf
				if (add_rs[i][0] == "+")
					add_bf[6] = to_string(stoi(add_rs[i][1]) + stoi(add_rs[i][2]));  //算出buffer裡面的值，存在第6項
				if (add_rs[i][0] == "-")
					add_bf[6] = to_string(stoi(add_rs[i][1]) - stoi(add_rs[i][2]));  //算出buffer裡面的值，存在第6項
				/*清空rs*/
				for (int a = 0; a < 6; a++) {
		...
		for (int i = 4; i < 6; i++) {
			if (mul_rs[i][1] == "")
				continue;
			if (mul_rs[i][1][0] != 'R'&&mul_rs[i][2][0] != 'R'&&stoi(mul_rs[i][5]) < cycle&&mul_bf[0] == "") {
				mul_bf[0] = "RS" + to_string(i);
				mul_bf[1] = mul_rs[i][1];
				mul_bf[2] = mul_rs[i][0];
				mul_bf[3] = mul_rs[i][2];
				mul_bf[4] = mul_rs[i][3];//存要放回RF哪一項  如果是存回R1就存1
				if (mul_rs[i][0] == "*") {
					mul_bf[5] = to_string(cycle + 10);    //第五項存第幾個cycle應該要離開buffer放回rf
					mul_bf[6] = to_string(stoi(mul_rs[i][1]) * stoi(mul_rs[i][2]));  //計算結果存在第6項
				}
				if (mul_rs[i][0] == "/") {
					mul_bf[5] = to_string(cycle + 40);  //第五項存第幾個cycle應該要離開buffer放回rf
					mul_bf[6] = to_string(stoi(mul_rs[i][1]) / stoi(mul_rs[i][2]));
				}
				/*清空rs*/
				for (int a = 0; a < 6; a++) {
					mul_rs[i][a] = "";
...
//----dispatch&放入buffer end
//----capture
		for (int i = 1; i <= 3; i++) {   //去rs裡面找有沒有buffer裡面存的rs幾，有的話可以去替換成計算完的值
			if (add_rs[i][1] == add_bf[0]) {//&& add_rs[i][5]!= to_string(cycle)
				add_rs[i][1] = add_bf[6];
			}
			if (add_rs[i][1] == mul_bf[0] ) {
				add_rs[i][1] = mul_bf[6];
			}
			if (add_rs[i][2] == add_bf[0] ) {
				add_rs[i][2] = add_bf[6];
			}
			if (add_rs[i][2] == mul_bf[0]) {
				add_rs[i][2] = mul_bf[6];
			}
		}
		for (int i = 4; i < 6; i++) {
			if (mul_rs[i][1] == mul_bf[0] ) {//&& mul_rs[i][5] != to_string(cycle)
				mul_rs[i][1] = mul_bf[6];
			}
			if (mul_rs[i][1] == add_bf[0] ) {
				mul_rs[i][1] = add_bf[6];
			}
			if (mul_rs[i][2] == mul_bf[0] ) {
				mul_rs[i][2] = mul_bf[6];
			}
			if (mul_rs[i][2] == add_bf[0]) {
				mul_rs[i][2] = add_bf[6];
			}
		}
//----capture  end
//----write result to rf
		...
		if (add_bf[0] != ""&&stoi(add_bf[5]) == cycle) {  //bffer有東西，且現在的cycle是他可以write的cycle時
			//string temp = "RS";
			//temp += add_bf[4];
			if (add_bf[0] == rat[add_bf[4][0]-'0']) {    //目標是要寫入到F4，如果F4現在存rs2且buffer就是存rs2
				rf[add_bf[4][0] - '0'] = stoi(add_bf[6]);    //就去將計算後的值寫回rf
			}
			for (int i = 0; i < 10; i++) {    //寫入後清空buffer
				add_bf[i] = "";
			}
		}
		if (mul_bf[0] != ""&&stoi(mul_bf[5]) == cycle) {
			if (mul_bf[0] == rat[mul_bf[4][0] - '0'])
				rf[mul_bf[4][0]-'0'] = stoi(mul_bf[6]);   //計算後的值寫回rf
			for (int i = 0; i < 10; i++) {    //清空buffer
				mul_bf[i] = "";
			}
		}
//----write result to rf
			...
			cout << "RS:" << endl;
			for (int i = 1; i <= 3; i++) {
				cout << "RS" << i << ":" << add_rs[i][0] << "   |" << add_rs[i][1] << "   |" << add_rs[i][2] << endl;
			}
			cout << "------------" << endl;
			cout << "BUFFER:" << add_bf[0] << "  " << add_bf[1] << "  " << add_bf[2] << "  " << add_bf[3] << "  " << endl;
			cout << "------------" << endl;
			for (int i = 4; i < 6; i++) {
				cout << "RS" << i << ":" << mul_rs[i][0] << "   |" << mul_rs[i][1] << "   |" << mul_rs[i][2] << endl;
			}
			cout << "------------" << endl;
			cout << "BUFFER:" << mul_bf[0] << "  " << mul_bf[1] << "  " << mul_bf[2] << "  " << mul_bf[3] << "  " << endl;;
			if (add_rs[1][0] == "" && add_rs[2][0] == "" &&
				add_rs[2][0] == "" && add_bf[0] == "" && mul_rs[4][0] == "" && mul_rs[5][0] == "" && mul_bf[0] == "") {
				end = true;
			}
		cycle++;
		
	}
//還要把Buffer的值放進rs裡面(capture)，跟buffer時間到放回rf
}
void find_label(string inst[][20], int address)
{
